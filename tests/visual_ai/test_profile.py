...
def test_profile_visual_integrity(page):
    login_page = LoginPage(page)
    login_page.api_login(os.getenv("ADMIN_EMAIL"), os.getenv("ADMIN_PASS"))
    login_page.navigate("#/profile")
    # page.locator("button:has-text('dark_mode')").click()
    page.wait_for_load_state("networkidle")
    current_shot = "testim/test/test_profile.png"
    page.screenshot(path=current_shot)

    similarity = SSIMChecker.check_layout(
        baseline_path="testim/baselines/win_profile_baseline.png", 
        current_path=current_shot,
        diff_save_path="testim/errors/diff_profile.png"
    )
    assert similarity >= 95.0, f"🚨 레이아웃 깨짐! (유사도: {similarity:.2f}%) errors 폴더를 확인하세요."
    ai = AIVerifier()

    required_classes = {0,2,3} 

    boxed_result_shot = "testim/debug/test_profile_boxed_success.png"
