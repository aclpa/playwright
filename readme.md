...
이 프로젝트는 일부 동작 방식을 차용해 Locator 실패 시 스크린샷을 촬영하고, 학습된 YOLO 모델로 UI 요소를 식별한 뒤 OCR로 요소 안의 텍스트를 추출합니다. 추출된 텍스트를 NLP로 기존 목표 텍스트와 비교하여 가장 유사도가 높은 요소의 좌표를 클릭합니다. 틀린 locator를 로그로 보여줍니다. 즉 1,2단계의 기능을 구현하고 수정방법을 제시합니다.
