...
- team, project, sprint, kanban 등 전 영역 E2E 테스트
- Locator 실패 시 AI 자가 복구 (YOLO 탐지 → OCR 텍스트 추출 → NLP 유사도 계산 → 좌표 클릭)
- 복구 리포트 자동 생성

> **참고**: Playwright 기반 테스트는 공식 문서를 참고하여 구현하였으나, **utils/YOLO, healer, NLP, labeler 통합 로직 AI 엔진 구현은 LLM의 도움을 받아 진행**하였습니다.
...
**API POST로 사전 데이터 생성 → Playwright 테스트 수행 → API DELETE로 정리**하는 하이브리드 방식을 도입하여 테스트 유연성과 안정성을 크게 높였습니다.

### 2. YOLO 모델 학습
UI 요소 탐지 학습에 필요한 100~300장의 데이터 수집 설계를 다음과 같이 진행하였습니다.
- Playwright `boundingBox()`를 활용해 자동 레이블링 데이터 수집  
